/**
 * Test Script: QR API Integration (End-to-End)
...
async function testQRAPIIntegration() {
    ...
    try {
        ...
        if (server && server._router) {
            ...
            // Check if QR routes are mounted
            const routeStack = server._router.stack;
            const qrRouteFound = routeStack.some(layer => {
                return layer.regexp && layer.regexp.source.includes('qr-codes');
            });
            
            if (qrRouteFound) {
                console.log('   ✅ QR routes are connected to main Express app');
                testResults.routesConnected = true;
            } else {
                console.log('   ❌ QR routes not found in Express app');
            }
        }
        ...
        try {
            ...
            // We'll just test the service components since we don't have a live database
            ...
            const mockRes = {
                status: (code) => ({
                    json: (data) => {
                        ...
                        if (code === 400 && data.message.includes('not found')) {
            ...
            const mockRes = {
                status: (code) => ({
                    json: (data) => {
                        ...
                        if (code === 404 && data.message.includes('not found')) {
                            // Expected for mock data
                            downloadTested = true;
                        }
                        return mockRes;
                ...
                setHeader: () => mockRes,
                sendFile: () => console.log('   📝 File download attempted'),
                headersSent: false
            };
